...
public interface StatServiceRepository extends JpaRepository<EndpointHit, Integer> {
    ...
    @Query("select new ru.practicum.ewm.dto.stats.ViewStatsDto(eh.app, eh.uri, count(eh.ip)) " +
    ...
    @Query("select new ru.practicum.ewm.dto.stats.ViewStatsDto(eh.app, eh.uri, count(distinct eh.ip)) " +
            "from EndpointHit as eh " +
            "where eh.timestamp between ?1 and ?2 and eh.uri IN ?3 " +
            "group by eh.app, eh.uri  " +
    ...
    @Query("select new ru.practicum.ewm.dto.stats.ViewStatsDto(eh.app, eh.uri, count (eh)) " +
            "from EndpointHit as eh " +
